...
const stringifyValue = (value: unknown) => {
  return _isNil(value) ? '' : JSON.stringify(value)
}

const parseValue = <T>(value: string | null | undefined) => {
  return _isNil(value) ? undefined : (JSON.parse(value) as T)
}
...
export const createRedisStore = (redisClient: RedisClientType, options?: RedisStoreOptions) => {
  ...
  const set = async (key: string, value: any, ttl?: number): Promise<void> => {
    ...
    const _ttl = _isUndefined(ttl) ? options?.defaultTTL : ttl
    if (!_isNil(_ttl) && _ttl !== 0) {
