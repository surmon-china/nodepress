...
export class SyndicationService {
  private pagesMap: SitemapItemLoose[] = [
    ...
    {
      url: urlMap.getAboutPageUrl(),
      changefreq: EnumChangefreq.YEARLY,
      priority: 1,
    },
    {
      url: urlMap.getArchivePageUrl(),
      changefreq: EnumChangefreq.ALWAYS,
      priority: 1,
    },
    {
      url: urlMap.getGuestbookPageUrl(),
      changefreq: EnumChangefreq.ALWAYS,
      priority: 1,
    },
  ]
  ...
  private getAllTags(): Promise<Tag[]> {
    return this.tagModel.find().sort({ _id: ESortType.Desc }).exec()
  }

  private getAllCategories(): Promise<Category[]> {
    return this.categoryModel.find().sort({ _id: ESortType.Desc }).exec()
  }
  ...
  private async getMapXML(): Promise<string> {
    try {
      ...
      const xmlData = await streamToPromise(Readable.from([...this.pagesMap, ...sitemapItemList]).pipe(sitemapStream))
