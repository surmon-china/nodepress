...
export class CommentService {
  ...
  private submitCommentAkismet(
    action: EAkismetActionType, comment: Comment, permalink?: string, referer?: string,
  ): Promise<any> {
  ...
  // 检查评论是否匹配系统的黑名单规则
  private validateCommentByBlacklist(comment: Comment): Promise<any> {
    return this.optionModel.findOne().exec().then(({ blacklist }) => {
      // 使用设置的黑名单 IP/邮箱/关键词 过滤
  ...
  private validateCommentByAkismet(comment: Comment, permalink: string, referer: string): Promise<any> {
  ...
  // 请求评论列表
  public getCommentsList(querys, options): Promise<PaginateResult<Comment>> {
    return this.commentModel.paginate(querys, options);
  }

  // 创建评论
  public createComment(comment: Comment, { ip, ua, referer }): Promise<Comment> {
  ...
  // 批量修改评论
  public patchCommentsState(action: PatchComments, referer: string): Promise<any > {
  ...
  // 获取单个评论详情
  public getItem(commentId: Types.ObjectId): Promise<Comment > {
    return this.commentModel.findById(commentId).exec();
  }

  // 修改评论
  public putItem(commentId: Types.ObjectId, newComment: Comment, referer: string): Promise<Comment> {
    return this.commentModel
  ...
  // 删除单个评论
  public deleteItem(commentId: Types.ObjectId): Promise<any> {
    return this.getItem(commentId).then(({ post_id }) => {
      this.updateCommentCountWithArticle([post_id]);
      return this.commentModel.findByIdAndRemove(commentId).exec();
    });
  }

  // 批量删除评论
  public deleteList(commentIds: Types.ObjectId[], postIds: number[]): Promise<any> {
    return this.commentModel
